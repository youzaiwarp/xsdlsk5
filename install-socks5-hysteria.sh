...
# 介绍信息
{
    ...
    echo "    _(__  ) / /_/ // /__  _  ,<   _(__  )  ____/ /        开启代理"
    echo "    /____/  \\____/ \\___/  /_/|_|  /____/  /_____/                 "
    echo "作者：苍井空 原作者们：西门庆"
    echo "葵花宝典:老汉推车"
    echo -e "\e[0m"  
}
...
# 设置服务器端口函数
set_server_port() {
  read -p "苍井空提示请输入 hysteria2 端口 (面板开放的UDP端口,默认 20026）: " input_port
...
# 安装和配置 socks5
socks5_config(){
  # 苍井空提示用户输入 socks5 端口号
  read -p "苍井空提示请输入 socks5 端口 (面板开放的TCP端口): " SOCKS5_PORT

  # 苍井空提示用户输入用户名和密码
  read -p "苍井空提示请输入 socks5 用户名: " SOCKS5_USER

  while true; do
    read -p "苍井空提示请输入 socks5 密码（不能包含@和:）：" SOCKS5_PASS
    echo
